...
\section{Groups and Their Basic Properties}

\subsection{Definition and Examples}

\subsubsection{Group Axioms}

\begin{eg} $S_n, A_n, D_n, C_n, (C_p)^n, GL(n, \mathbb{F}), SL(n, \mathbb{F})$ \end{eg}

\begin{Def} \textbf{Order} \end{Def}
\begin{Def} \textbf{p-group} \end{Def}

\begin{Def}
V is an \textbf{elementary $p$-group} if $|g| \, \big|\, p$ for all $g \in V$
\end{Def}



\subsection{Subgroups, Cosets and Normality}

\begin{Def} \textbf{Subgroup} \end{Def}
\begin{Def} \textbf{Centre} \end{Def}
\begin{Def} \textbf{Commutator Subgroup} \end{Def}

\begin{Def} \textbf{Maximal Subgroups} \end{Def}
...
\begin{Def} For $g, h \in G$, we define \textbf{Conjugation} of $g$ by $h$ as $g^h = h^{-1} g h$. \end{Def}

\begin{Def} For $g \in G$, we define its \textbf{Conjugacy Class} as $cl(x) = \{ a \in G : a = g^h, h \in G \}$. \end{Def}


\begin{Def} We call a group $G$ \textbf{Simple} if it has no non trivial proper normal subgroups: if $N \triangleleft G$ then either $N = 1$ or $N = G$. \end{Def}
\begin{Def} \textbf{Minimal Normal Subgroup} \end{Def}


\begin{Def} \textbf{Products of Subgroups} \end{Def}



\begin{Def} \textbf{Characteristics} \end{Def}

\begin{Def} \textbf{Centraliser} \end{Def}
...
\[ \phi(xy) = \phi(x) \phi(y) \]
We call a surjective homomorphism an \textbf{Epimorphism}, and a bijective homomorphism an \textbf{Isomorphism}. If $\phi : G \rightarrow G$ is an isomorphism then we call it an \textbf{Automorphism}.
\end{Def}

\begin{Def}
For a homomorphism $\phi : G \rightarrow H$, we define its \textbf{Kernel} as $\ker \phi = \{ g \in G : \phi(g) = 1 \}$.
\end{Def}
...
\begin{Def}
If $H \triangleleft G$ then we define the \textbf{Factor Group} $G / H = \{Hx : x \in G\}$, i.e. the set of (right) cosets of $H$.
\end{Def}
...
\begin{eg} Let $N \triangleleft G$. Define $\phi : G \rightarrow G / H$ by $g \mapsto Ng$. Observe this is a homomorphism. In fact, we call this the \textbf{Canonical Homomorphism} \end{eg}
...
We call this the \textbf{External Direct Product}, and it is a group under the operation $(g,h) \cdot (a,b) = (ga, hb)$. We want to understand when a group $G$ can be written as a direct product $G \simeq H \times K$, for some $H, K \leq G$.  
\end{Def}
...
%OTHER THINGS THAT NEED TO BE DEFINED IN THIS SECTION
...
\end{enumerate}
Then we say $G$ \textbf{Acts} on $\Omega$, and $\cdot$ is an \textbf{Action} of $G$ on $\Omega$
\end{Def}

\begin{eg}
Take $\Omega \neq \varnothing$, $G \leq \text{Sym}(\Omega)$ then for all $\alpha \in \Omega$ and all $g \in G$, $\alpha \cdot g  = \alpha^g$ image of $\alpha$ under $g$. (1) is satisfied by definition of the identity of Sym($\Omega$). (2) is satisfied by the definition of multiplication in Sym($\Omega$).
\end{eg}
...
%Let $x \in O$. By the fundamental counting principle, $|O| |G_x| = |G|$. Considering divisibility of $|G|$ by $p^a$ gives $p^a \big| \, |G_x|$ so $p^a \leq |G_x|$. Now for any $X \in \Omega$ with $X \supseteq \{x\}$ and $y \in X$ and $h \in G_x$, $yh \in Xh = X$. Thus $xG_x \subseteq X$, so $|G_x| = |xG_x| \leq |X| = p^a$, and hence $|G_x| \leq p^a$. Thus $G_x$ has order $p^a$, giving us the desired subgroup.    
\end{proof}
...
\begin{Cor}
$n_p(G) = | G : N_G(P) |$ for $P \in \text{Syl}_p(G)$. Also $n_p(G) \,  \big| \, |G :P|$.
\end{Cor}
...
\newpage
\section{Free Groups and Group Presentations}
\begin{Def}
...
\begin{eg}
If $X = \{ x , y\}$ then some words are: $xyx^{-1}y, \qquad yx^{-1}yxy^{-1}$
\end{eg}
...
\begin{Def}
The \textbf{Free Group} on a non-empty set $X$ is the set of equivalence classes of words on $X \cup X^{-1}$ with the product given above. We often write this as $F(X)$ or just $F$ when $X$ is clear from context.
\end{Def}


\begin{Def}
A word in $X \cap X^{-1}$ is called \textbf{Reduced} if it has the form $x_{\lambda_1}^{\epsilon_1}x_{\lambda_2}^{\epsilon_2}\cdots x_{\lambda_n}^{\epsilon_n}$ where $x_{\lambda_i}^{\epsilon_i} \neq x_{\lambda_{i+1}}^{-\epsilon_{i+1}}$ for all $i < n-1$. 
\end{Def}
...
\begin{Def}
A group $F$ is \textbf{Free} on $X \subseteq F$ if, given an arbitrary group $H$ and map $\theta: F \rightarrow H$, there exists a unique homomorphism $\theta' : F \rightarrow G$ extending $\theta$. 
\end{Def}
...
\begin{Def}
Let $X$ be a set and let $\Delta$ be a set of words on $X \cup X^{-1}$. A group $G$ has \textbf{Generators} $X$ and \textbf{Relators} $\Delta$ if $G \simeq F / R$ where $F = F(X)$ and $R$ is the normal closure of $\Delta$ (i.e., the smallest normal subgroup of $F$ containing $\Delta$). We say $G$ has \textbf{Presentation} $\{ X : \Delta \}$ and $G = \langle X : \Delta \rangle$. If $|X|$ and $|\Delta|$ are finite, then we say that $G$ is \textbf{Finitely Presented}.
\end{Def}
...
We claim $|H| = 16$. We also claim that $A^4 = B^2$ and $B^{-1} A B = A^{-1}$. Thus we can use Von Dyck's Theorem, and see that $|G| = 16$. 
\end{eg}
...
We shall prove this later once we have considered semi direct products.

\begin{eg}[Classification of 2-generated groups of order 8]
...
\begin{Def}
Take an abelian group $G$. Take $x_1, \dots x_r \in G$. Then any integral linear combination (i.e, a linear combination over $\ZZ$) is an element of $G$. Define a \textbf{Basis} for $G$ as a generating set $X$ such that any finite sequence of elements of $X$, the only vanishing integral linear combination is the trivial one. If an abelian group has a basis, then it is a \textbf{Free Abelian Group}.
\end{Def}
...
\begin{Lem}
$G_{ab} = G / G'$ is abelian. If $N \triangleleft G$, $G / N$ is abelian if and only if $N \geq G'$. Thus $G / G'$ is the largest abelian quotient of $G$.  
\end{Lem}
...
\subsection{Characterisation of Finitely Generated Abelian Groups}
...
\begin{Def}
Two integral matrices $A$ and $B$ are \textbf{Equivalent} (over $\ZZ$) if one can be obtained from the other using elementary integral row and column operations.  We write $A \sim B$. 
\end{Def}
...
\begin{Def}
If $G$ has two subnormal series, then we say that the second is a \textbf{Refinement} of the first if each member of the first is also a member of the second.
\end{Def}
...
\begin{Def}
A \textbf{Composition Series} of $G$ is a subnormal series without repetitions which cannot be further refined. 
\end{Def}
...
\begin{rem}
It is not the case that knowledge of the composition factors gives knowledge of the group isomorphism type. For example, $\ZZ_4 \triangleright \ZZ_2 \triangleright 1$ and $\ZZ_2 \times \ZZ_2 \triangleright \ZZ_2 \triangleright 1$ have the same composition factors.
\end{rem}
...
\begin{Lem}
A nontrivial simple abelian group is cyclic of prime order.
\end{Lem}

%\begin{proof}
%If $G$ is a simple abelian group, then for $1 \neq x \in G$ we have that both $\langle x \rangle, \langle x^2 \rangle \triangleleft G$ and $\langle x^2 \rangle \leq \langle x \rangle$. But $G$ is simple, so there are two cases: if $\langle x^2 \rangle  = 1$ then $G = \ZZ_2$; else $\langle x^2 \rangle = \langle x \rangle$, so $x \in \langle x^2 \rangle$ and thus $x^{2n} = 1$ for some $n \in \NN$. Thus $|x|$ is finite and so, since , $G$ is finite. Take $p | |G|$. Then there exists some $H \triangleleft G$ with $|H| = p$ and so $|G| = p$.
%\end{proof}
...
\begin{Def}
A group $G$ is \textbf{Soluble} (also sometimes \textbf{Solvable}) if it has a normal series:
\[ G = G_0 \triangleright G_1 \triangleright \cdots \triangleright G_r = 1 \]
...
is an abelian series for $G$ of finite length, and so $G$ is soluble.
\end{proof}
...
\begin{Lem}
%TODO: Is a minimal normal subgroup non trivial?
...
\subsubsection{General Central Series}
...
\subsubsection{Largest Nilpotent Subgroup}
...
\subsubsection{Frattini Subgroup}
...
Recall that one-point stabilisers of a transitive action are conjugate.
...
\newpage
\section{Groups Constructions}

We have already seen that the direct product allows us to form new groups, and if:
\begin{enumerate}
...
Since these external semi direct products rely heavily on understanding automorphism groups and homomorphisms to them we make the following remarks:
\begin{rem}
...
\subsubsection{Affine General Linear Group}
...
\subsubsection{Orders of Iterated Wreath Products}
...
\subsubsection{General Construction of Wreath Products}
