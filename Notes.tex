...
\begin{proof}
If $p = 2$, then $|G| = 4$ for which the claim is true. Otherwise, $p$ is odd and so $G$ has an element $s$ of order $p$ and an element $t$ of order $2$. Let $H = \langle s \rangle$, so $|G:H| = 2$ and $H \triangleleft G$. Thus $t^{-1} s t = tst = s^i$ for some $i$. Thus $s = t^2 s t^2 = t s^i t = s^{i^2}$, and so $i^2 \equiv 1 \mod p$. But as $p$ is prime, $1 \equiv \pm 1 \mod p$. Therefore $tst = s$ or $tst = s^{-1}$. In the first case, 
