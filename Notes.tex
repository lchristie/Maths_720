...
\begin{Lem}
Take homomorphism $\phi : G_1 \rightarrow G_2$ where $\phi(N_1) = N2$ for $N_i \triangleleft G$, ($i = 1,2$). Then there exists a homomorphism $\psi : G_1 / N_1 \rightarrow G_2 / N_2$ defined by:
\[ (N_1x)\psi = N_2(x\phi) \]
for all $x \in G_1$.   
\end{Lem}
...
\begin{thm}
If $H \leq G$, and $|G : H| = n < \infty$, there exists $N \triangle G$ such that $N \leq H$ and $|G : N| \big| n!$, if $n > 1$ and $|G| \nmid n!$, then $G$ is not simple.
\end{thm}
...
\\
If $n > 1$: then $H < G$ and so $N < G$. If $n = 1$ then $|G| = |G / N| \big| n!$. This proves $N > 1$ so $G$ is not finite.
\end{proof}
...
\begin{rem}
Take $H \leq G$ with $|G:H| < \infty$. Then there exists $N \triangle G$ of finite index with $N \leq H$. We call
\[ N = \text{core}_G(H), \]
